...
{
    /// <summary>
    /// The product of diagonal numbers is 26 × 63 × 78 × 14 = 1788696.
    /// 
    /// What is the greatest product of four adjacent numbers in the same direction(up, down, left, right, or diagonally) in the 20×20 grid?
    /// </summary>
    public class Task11
    {
        readonly int _productLength;
        ...
        {
            new Tuple<int, int>(1, 1), //diagonal
            new Tuple<int, int>(1, 0), // down
            new Tuple<int, int>(0, 1), // right
            new Tuple<int, int>(1, -1)// another diagonal
        };

        public Task11(int productLength)
        {
            _productLength = productLength;
        }

        public int GetProduct(int row, int column, int rowDirection, int columnDirection)
        {
            if (row + rowDirection * _productLength >= MATRIX.Length || column + _productLength * columnDirection >= MATRIX[0].Length
                || row + rowDirection * _productLength < 0 || column + _productLength * columnDirection < 0)
                return -1;

            int product = 1;
            for (int i = 0; i < _productLength; ++i)
            {
                product *= MATRIX[row + i * rowDirection][column + i * columnDirection];
            }
            return product;
        }
        ...
        public int Run()
        {
            ...
            for (int row = 0; row < MATRIX.Length; ++row)
            {
                for (int column = 0; column < MATRIX[0].Length; ++column)
                {
                    ...
                    {
                        tempProduct = GetProduct(row, column, direction.Item1, direction.Item2);
